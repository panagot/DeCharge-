...
export function GoogleMapsView() {
  ...
  // Default to Washington, D.C. for clarity, user can pan/zoom freely
  ...
  const cpToPlot = window.__zustand ? undefined : undefined;
  ...
  return (
    <div className="panel">
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
        ...
        <label className="muted" style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          Min kW
          <input type="number" value={filters.minKw} min={0} step={1} style={{ width: 70, background: '#0b1220', color: '#e2e8f0', border: '1px solid #223049', borderRadius: 6, padding: '4px 6px' }} onChange={e => setFilters({ ...filters, minKw: Number(e.target.value || 0) })} />
        </label>
      </div>
      {apiKey && (
        <LoadScript googleMapsApiKey={apiKey} libraries={["visualization"]}>
          ...
          >
            {/* Heatmap weighted by connector kW and a simple queue estimate (active session on linked plot) */}
            <HeatmapLayer
              data={charge_points.map(cp => ({
              ...
              options={{ radius: 24, dissipating: true }}
            />
            {charge_points
                    ...
                    return (
                      <div style={{ marginTop: 8, paddingTop: 8, borderTop: '1px dashed #1b2635' }}>
                        ...
                        {active && (
                          <div style={{ marginTop: 6 }}>
                            <a
                              href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(`Charging at ${selected.name} • ${kwh} kWh • ${costPts} POINTS • #DePIN @DeCharge`)}`}
      ...
      <div className="muted" style={{ marginTop: 8 }}>Data source: DeCharge sample. Map for visualization only.</div>
    </div>
  );
}
